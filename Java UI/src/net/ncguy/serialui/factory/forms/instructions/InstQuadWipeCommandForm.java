...
import net.ncguy.serialui.cmd.InstQuadWipeCommand;
 ...
 */
public class InstQuadWipeCommandForm {
    ...
    public InstQuadWipeCommandForm(RGBForm host) {
        this.host = host;
        setColourABtn.addActionListener(actionEvent -> {
            Color col = setColourABG.getBackground();
            col = JColorChooser.showDialog(InstQuadWipeCommandForm.this.root, "Choose a colour", col);
            if(col == null) return;
        ...
        sendWipe.addActionListener(actionEvent -> {
            ...
            try {
                host.SendCommandPayload(new InstQuadWipeCommand().PreparePayload(data));
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
