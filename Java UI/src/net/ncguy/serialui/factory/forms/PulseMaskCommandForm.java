...
import net.ncguy.serialui.cmd.PulseMaskCommand;
import net.ncguy.serialui.controls.PixelMaskForm;

import javax.swing.*;
import java.awt.*;
import java.io.IOException;

import static net.ncguy.serialui.RGBForm.PIXELCOUNT;

/**
 * Created by nick on 07/05/17.
 */
public class PulseMaskCommandForm {
    public JPanel root;
    ...
    private PixelMaskForm pixelMask;
    private RGBForm host;

    public PulseMaskCommandForm(RGBForm host) {
        this.host = host;
        pixelMask.Initialize(PIXELCOUNT);
        setColourABtn.addActionListener(actionEvent -> {
            Color col = setColourABG.getBackground();
            col = JColorChooser.showDialog(PulseMaskCommandForm.this.root, "Choose a colour", col);
            if(col == null) return;
        ...
        sendPulse.addActionListener(actionEvent -> {
            ...
            boolean[] mask = pixelMask.ToMask();
            for(int i = 0; i < PIXELCOUNT; i++)
                data[6 + i] = mask[i] ? 1 : 0;

            data[data.length-1] = Math.max(1, Math.min((Integer) pulseTime.getValue(), 255));
            try {
                host.SendCommandPayload(new PulseMaskCommand().PreparePayload(data));
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
